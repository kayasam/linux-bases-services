...
Lors de l'installation, choisir **Internet Site**, et renseigner le nom de domaine interne `fournil.lab`.
...
```text
myhostname = mail.fournil.lab
mydomain = fournil.lab
...
echo "Ceci est un test" | mail -s "Sujet du test" bob@fournil.lab
...
```text
EHLO client-labo
MAIL FROM:<alice@fournil.lab>
RCPT TO:<bob@fournil.lab>
DATA
Subject: test SMTP
...
- Dans l'admin, section **Domaines** : créer le domaine `fournil.lab` avec IMAP = `localhost:143` (sans SSL) et SMTP = `localhost:25`
